...
CD8_clust_markers <- c("CCR7", "CD103", "CD27", "CD45R0", "CD45RA", "CD56")

data_heatmap <- data_cyto %>%
  dplyr::filter(main_clusters == "CD8") %>%
  dplyr::group_by(sub_clusters) %>%
  dplyr::summarise(across(.cols = all_of(CD8_clust_markers), median,
                          .groups = "drop_last")) %>%
  dplyr::ungroup() %>%
  tibble::column_to_rownames("sub_clusters") %>%
  as.matrix()

figS2.A <- Heatmap(data_heatmap,
                  col = white.black,
...
setwd("../figures")
pdf(file = "Figure_S2A.pdf", width = 10)
print(figS2.A)
invisible(dev.off())

rm(white.black, CD8_clust_markers, data_heatmap) # cleanup
```
...
data_violin <- data_cyto %>%
  ...
  dplyr::add_tally(name = "CD8_count") %>%
  dplyr::ungroup() %>%
  dplyr::group_by(patient_timepoint, sub_clusters) %>%
  dplyr::add_tally(name = "CD8sub_count") %>%
  dplyr::ungroup() %>%
  dplyr::select(patient_timepoint, sub_clusters, CD8sub_count,
                CD8_count) %>% # total_count
  dplyr::distinct() %>%
  dplyr::mutate(CD8_comp = 100 * CD8sub_count / CD8_count) %>%
...
figS2.B <- ggplot(data_violin, aes(x = treated, y = CD8_comp, fill = treated)) +
...
setwd("../figures")
pdf(file = "Figure_S2B.pdf", width = 14, height = 5)
print(figS2.B)
invisible(dev.off())
...
setwd("../figures")
pdf(file = "Figure_S2C.pdf", height = 8, width = 4)
print(figS2.C)
invisible(dev.off())
...
```{r, warning=FALSE}
cp <- c("#4545fc", "#FF6666", "#8CC09D", "#F0EF70", "#F0BA64", "#FDCFCA", "#ACDDFF","#8C9093")
...
figS2.E_temp <- ggplot(data_violin, aes(x = treated, y = CXCR5, fill = treated)) +
  # stat_compare_means(comparisons = list(c("Before", "After")), paired = T) +
  geom_violin(scale ="area", alpha = 1, draw_quantiles = .5) +
  geom_point(size = 0.5) +
  geom_line(aes(group = patient_id), colour="black", linetype="11") +
...
setwd("../figures")
pdf(file = "Figure_S2E_temp_CXCR5.pdf", width = 14, height = 4.5)
print(figS2.E_temp)
invisible(dev.off())
...
data_graph <- data_cyto %>%
  dplyr::filter(main_clusters == "CD8") %>%
  dplyr::filter(!sub_clusters %in% c("CD103+", "NKT")) %>%
  dplyr::group_by(patient_timepoint, sub_clusters) %>%
  ...
  dplyr::mutate(x_axis = case_when(
    ...
    sub_clusters == "Central Memory" ~ 2,
    sub_clusters == "Effector Memory" ~ 3,
    sub_clusters == "Effector" ~ 4,
    TRUE ~ NA_real_)) %>%
  dplyr::mutate(x_axis_shift = if_else(treated == "After", x_axis + 0.1,
                                       x_axis - 0.1, NA_real_))

figS2.E <- ggplot(data_graph,
                 aes(x = x_axis, y = CD27, col= treated)) +
  geom_line(aes(x = x_axis_shift, group = interaction(patient_id, sub_clusters)), colour="black", linetype="11") +
  geom_smooth(method = "loess", formula = "y ~ x", se = FALSE, size = 1) +
  geom_point(size = 1, aes(x = x_axis_shift)) +
  scale_x_continuous(breaks = seq(1, 4, 1),
                     labels = c("Naive", "Central Memory",
                                "Effector Memory", "Effector")) +
  scale_color_manual(values = c("#abc3c9", "#e0dcd3")) +
  ylab("median CD27 expression") +
  xlab("CD8+ T cell subsets") +
...
setwd("../figures")
pdf(file = "Figure_S2E.pdf", width = 12, height = 5)
print(figS2.E)
invisible(dev.off())
...
setwd("../figures")
pdf(file = "Figure_S2F.pdf", width = 12, height = 5)
print(figS2.F)
invisible(dev.off())
